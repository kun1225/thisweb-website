---
title: 'JavaScript 教學 中階篇 2 - 原型 (prototype) 與繼承 (inherit)'
...
我們在構造函數那篇文中提到了構造函數的問題，就是會在實例(被創造出來的物件稱為實例)中重複宣告同樣的方法會浪費資源，而原型 prototype 可以幫助我們來解決這個問題，原型也是面試很容易考到的觀念之一，所以對於想要更進階學習的前端新手來說，是一個逃不開的話題，那就讓我們一起來看看 JS 中的大魔王之一 - 原型吧！

## 原型 prototype 是什麼

簡單說，原型就像是每個實例的祖先，其實每一個函數都會**自動內建**一個 `prototype` 原型屬性，不信你可以去試試:
...
這個自動內建的屬性是一個**物件**，也就是常常聽到的**原型物件**，原型物件就像一個大倉庫，每個實例都可以來這個倉庫拿東西，所以只要把方法宣告放在原型物件裡，就解決了重複宣告的問題了。
...
### 所有函式都有 prototype 屬性，那 Object 也有嗎？
