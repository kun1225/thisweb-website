---
title: 'JavaScript 教學 入門篇 2 - 物件、陣列、函式'
desc: '在上一篇教學中講到了 JS 一些基本的用法，像是變數、型別、流程控制、迴圈，但除了這些，在JS 中還有三個不可或缺的東西，分別是物件、陣列、函數，這些是 JS 程式語言中最重要的一部份，寫程式時絕對脫離不了這三個要素，所以作為初學的新手也是必學的。'
date: '2023-08-02'
---

在上一篇教學中講到了 JS 一些基本的用法，像是變數、型別、流程控制、迴圈，但除了這些，在JS 中還有三個不可或缺的東西，分別是物件、陣列、函數，這些是 JS 程式語言中最重要的一部份，寫程式時絕對脫離不了這三個要素，所以作為初學的新手也是必學的。
...
<a
  href="https://thisweb.tech/js-constructor-function/"
  target="_blank"
  rel="noopener"
>
  JS 創建物件的模式 - 構造函式
...
<a
  href="https://thisweb.tech/javascript-var-let-const/"
  target="_blank"
  rel="noopener"
>
  JS 中 var、let、const 的差別
</a>
