...
export const POST_QUERY = groq`*[_type == "post" && defined(slug) && defined(title) && slug.current == $slug && status == 'done'][0]{..., "category": category->title, "subCategory": subCategory->title,"author": author->name, body[]{
  ...,
  markDefs[]{
    ...,
    _type == "internalLink" => {
...
export const RELATED_POSTS_QUERY = groq`*[_type == "post" && defined(slug) && defined(title) && status == 'done' && category->title == $categoryTitle && subCategory->title == $subCategoryTitle]{title, _id, slug} | order(publishedAt asc)`;
